        ...
        for fold in 1:nfolds
            ...
            @sync @simd for j in 1:n_ts
                Threads.@spawn begin 
                    ...
                    ts, pred_err, stats, _ = MPS_impute(fc, class, sample, impute_sites, :directMode; NN_baseline=false, X_train=Xs_train_fold, y_train=ys_train_fold,  plot_fits=false, mode_range=mode_range, xvals=xvals, mode_index=mode_index, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it);
...
#     impute_sites::Vector{<:Integer}
#     max_ts_per_class::Integer

#     function ImputationMeasureHack(Xs_train::Matrix, ys_train::Vector, impute_sites::Vector, max_ts_per_class::Integer)
#         if maximum(impute_sites) > size(Xs_train, 2) || minimum(impute_sites) < 1
#             error("Trying to impute a site not in the timeseries!")
#         end
#         return new(Xs_train, ys_train, impute_sites, max_ts_per_class)
#     end
# end

# ImputationMeasureHack(Xs_train::Matrix, ys_train::Vector, impute_sites::Vector) = ImputationMeasureHack(Xs_train, ys_train, impute_sites, 0)
...
#                 ts, pred_err, stats, _ = MPS_impute(fc, class, sample, heuristic.impute_sites, :directMode; NN_baseline=false, X_train=Xs_train, y_train=ys_train,  plot_fits=false, mode_range=mode_range, xvals=xvals, mode_index=mode_index, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it);
#                 f_per_fold[fold] += stats[:MAE]
#             end
#             f_per_fold[fold] /= n_ts
#             # p.counter += 1
