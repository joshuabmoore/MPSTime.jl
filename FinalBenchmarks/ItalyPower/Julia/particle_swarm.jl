...
train_interp=true
impute_sites= 5:15 |> collect # which sites to impute
max_samps=1#0 # the maximum number of test timeseries to impute/compute the MAE of
####################################
####################################
...
@everywhere function optimise_on_fold(train_idxs, test_idxs, Xs, ys)
    ...
    for j in 2:nCVevals
        if train_interp
            revise_history!(MMI.matrix(X_train_fold), Int.(int(y_train_fold)) .-1, impute_sites, max_samps, mach.report[:fit][:history][end-n_particles+1:end])
        end
