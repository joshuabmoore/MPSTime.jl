...
using JLD2
using ProgressMeter
dloc =  "/Users/joshua/Desktop/QuantumInspiredMLFinal/QuantumInspiredML/Data/Van_der_Pol/vdp_eta_01_c_1.0:2.0_k_1.0:2.0.jld2"
f = jldopen(dloc, "r")
...
# X_train_fold = vcat(X_train_fold_all[1]'...) # ts1
# y_train_fold = zeros(Int64, size(X_train_fold, 1))
# X_test_fold = vcat(X_test_fold_all[1]'...)
...
opts=MPSOptions(; nsweeps=2, chi_max=chi_max,  update_iters=1, verbosity=verbosity, loss_grad=:KLD,
...
@showprogress for t in 1:n_traj
    trajectories[t, :] = any_impute_ITS(fc, 0, 5, collect(30:100); X_train=X_train, y_train=y_train, rejection_threshold=1.25)
end
...
function make_trajectory_animation(trajectories::AbstractMatrix{<:Real}, n_trajectories::Int,
    ...
    anim = @animate for frame_num in 1:(n_trajectories * n_frames_per_traj)
        ...
        max_ylim = maximum(maximum(trajectories, dims=2)) # add 0.1 padding
        min_ylim = minimum(minimum(trajectories, dims=2)) # subtract 0.1 padding
...
anim = make_trajectory_animation(trajectories, 10, 30:100)
gif(anim, "vdp_trajectories_dataset2_s5.gif", fps = 50)    