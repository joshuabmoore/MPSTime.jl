...
dloc =  "Data/syntheticV2/complex/datasets/eta_0.1_m_disc_range_2_phi_cont_range_0_6.283185307179586_tau_disc_range_2.jld2"
f = jldopen(dloc, "r")
    X_train = read(f, "X_train")
    y_train = read(f, "y_train")
    X_test = read(f, "X_test")
    y_test = read(f, "y_test")
    train_metadata = read(f, "train_metadata")
    test_metadata = read(f, "test_metadata")
close(f)
...
opts=MPSOptions(; nsweeps=10, chi_max=chi_max,  update_iters=1, verbosity=verbosity, loss_grad=:KLD,
    bbopt=:TSGO, track_cost=track_cost, eta=0.3, rescale = (false, true), d=d, aux_basis_dim=2, encoding=encoding, 
...
svpath = "/Users/joshua/Desktop/QuantumInspiredML/Data/syntheticV2/complex/mps_saves/dataset1_legendre_ns_d$(d)_chi$(chi_max).jld2"
f = jldopen(svpath, "w")
    ...
    write(f, "mps", W)
    write(f, "opts", opts)
    write(f, "train_meta", train_metadata)
    write(f, "test_meta", test_metadata)
close(f)
