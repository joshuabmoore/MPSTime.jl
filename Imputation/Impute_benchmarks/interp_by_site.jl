...
@time begin 
    for ii in eachindex(nendpoints)
        n = nendpoints[ii]
        impute_sites = 1+n:nsites-n |> collect
        tasks = map(data_chunks) do chunk
            @spawn begin
                ...
                for (j, i) in enumerate(chunk) 
                    ...
                    ts, pred_err, stat, p1 = MPS_impute(fc, class, instance_idx, impute_sites, :directMedian; invert_transform=true, NN_baseline=false, X_train=X_train, y_train=y_train, n_baselines=1, plot_fits=false, dx=dx, mode_range=mode_range, xvals=xvals, mode_index=mode_index, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it, max_jump=max_jump);
                    stats_chunk[j] = stat
                end
                return stats_chunk
            end
...
svpath = "Imputation/impute_benchmarks/SL_ecg200_fitpersite.jld2"
f = jldopen(svpath, "w")
    write(f, "fc", fc)
    write(f, "stats", stats)
...
svpath = "Imputation/impute_benchmarks/SL_ecg200_fitpersite_nomps.jld2"
f = jldopen(svpath, "w")
    write(f, "stats", stats)
    write(f, "nendpoints", nendpoints)
close(f)
...
#             class = classes[i]
#             instance_idx = samples[i]
#             ts, pred_err, stat1, p1 = MPS_impute(fc, class, instance_idx, impute_sites, :directMedian; invert_transform=true, NN_baseline=false, X_train=X_train, y_train=y_train, n_baselines=1, plot_fits=false, dx=dx, mode_range=mode_range, xvals=xvals, mode_index=mode_index, xvals_enc=xvals_enc, xvals_enc_it=xvals_enc_it, max_jump=max_jump);
#             # push!(ps1, p1...)
#             stats[ii][i] = stat1
#         end
