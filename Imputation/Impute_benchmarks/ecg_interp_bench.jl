...
function midpoint_benchmark(ys_test, fc, impute_sites, Xs_train, ys_train)
    n1s = sum(ys_test)
    ...
    for (i,s) in enumerate(samples)
        class = classes[i]
        instance_idx = s
        stat, MSE_stat, p1 = any_impute_single_time_series(fc, class, instance_idx, impute_sites, :directMode; MSE_baseline=true, X_train_scaled=Xs_train, y_train=ys_train);
        push!(ps, p1)
        push!(stats, stat)
...
function make_impute_sites(n_mid::Integer, Xs::Matrix{Float64})
    npoints = size(Xs, 2)
...
for i in eachindex(n_midpoints)
    n_mid = n_midpoints[i]
    impute_sites = make_impute_sites(n_mid, X_train_scaled)
    stats[i], MSE_stats[i], ps[i] = midpoint_benchmark(y_test, fc, impute_sites, X_train_scaled, y_train)
    
end

idstr = "ecg_"*prod(map(x-> string(x) * "_", n_midpoints))[1:end-1] *".jld2"

path = "Imputation/impute_benchmarks/midpoint_benchmarks/" * idstr
